...
"""
    ...
    <gui fullscreen='0'>
      <camera name='user_camera'>
        <pose>6.6801 -4.45717 2.91151 0 0.291643 2.47219</pose>
...
def generate_world(args):
    positions = generate_spawns(args.cnt, args.dynamic_test)
    with open(PATH, 'w') as f:
        ...
        for name, pos in positions:
            f.write(model_state.format(name, pos))
        f.write(end)
...
def generate_dynamic_test_spawns(cnt):
    ...
    poses = ["0 0 {} 0 0 0".format(x) for x in ids]
...
def run_gazebo():
    cmd = ["gazebo", PATH, "--pause"]
    p = subprocess.Popen(cmd, stdout=subprocess.PIPE)

def main():
    parser = argparse.ArgumentParser()
    parser.add_argument("cnt", type=int, default=10)
    parser.add_argument("dynamic_test", type=bool, default=False)
